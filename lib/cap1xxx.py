...
class Cap1xxx():
    ...
    def __init__(self,
                 i2c, i2c_addr=DEFAULT_ADDR,
                 alert_pin=-1, reset_pin=-1, on_touch=None, skip_init=False):

        self.i2c_addr   = i2c_addr
        self.i2c_device = I2CDevice(i2c,i2c_addr)

        if not alert_pin == -1:
            self.alert_pin = digitalio.DigitalInOut(alert_pin)
            self.alert_pin.direction = digitalio.Direction.INPUT
            self.alert_pin.pull      = digitalio.Pull.UP
        else:
            self.alert_pin = None
        ...
        if not self.product_id in self.supported:
            raise Exception(f"Product ID (0b{self.product_id:b}) not supported!")
    ...
    def _interrupt_status(self):
        if not self.alert_pin:
            ...
            return not self.alert_pin.value
...
def DetectCap(i2c_addr,i2c,product_id):
    i2c_device = I2CDevice(i2c,i2c_addr)
    try:
        with i2c_device as i2c:
            result = bytearray(1)
            i2c_write_then_readinto(bytes([R_PRODUCT_ID]), result)
